...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		if(doPush) 
			luaArray.push(new FunkinLua(Asset2File.getPath(luaFile)));
				    ...
				    #if (LUA_ALLOWED)
		var doPush:Bool = false;
		var luaFile:String = 'data/' + Paths.formatToSongPath(SONG.song) + '/pause.lua';
			luaFile = Paths.getPreloadPath(luaFile);
			if(OpenFlAssets.exists(luaFile)) {
				doPush = true;
			}
		...
		if(doPush) 
			luaArray.push(new FunkinLua(Asset2File.getPath(luaFile)));
				    ...
				    #if (LUA_ALLOWED)
		var doPush:Bool = false;
		var luaFile:String = 'data/' + Paths.formatToSongPath(SONG.song) + '/script2';
			luaFile = Paths.getPreloadPath(luaFile);
			if(OpenFlAssets.exists(luaFile)) {
				doPush = true;
			}
		...
		if(doPush) 
			luaArray.push(new FunkinLua(Asset2File.getPath(luaFile)));
				    ...
				    #if (LUA_ALLOWED)
		var doPush:Bool = false;
		var luaFile:String = 'data/' + Paths.formatToSongPath(SONG.song) + '/limit misses.lua';
			luaFile = Paths.getPreloadPath(luaFile);
			if(OpenFlAssets.exists(luaFile)) {
				doPush = true;
			}
		...
		if(doPush) 
			luaArray.push(new FunkinLua(Asset2File.getPath(luaFile)));
				    ...
				    #if (LUA_ALLOWED)
		var doPush:Bool = false;
		var luaFile:String = 'data/' + Paths.formatToSongPath(SONG.song) + '/warning.lua';
			luaFile = Paths.getPreloadPath(luaFile);
			if(OpenFlAssets.exists(luaFile)) {
				doPush = true;
			}
		...
		Lib.application.window.title = "Friday Night Funkin': Basement Show - " + daSong.toUpperCase();
	...
	override function destroy() {
		Lib.application.window.title = "Friday Night Funkin': Basement Show";
		for (lua in luaArray) {
			lua.call('onDestroy', []);
			lua.stop();
		}
